...
func GetPostById(ctx context.Context, pid int64) (post *models.Post, err error) {
	...
	sqlStr := `select post_id, title, content, author_id, community_id, like_count, create_time 
				from post 
				where post_id = ?`
...
func GetPostListByIDs(ctx context.Context, ids []string) (postList []*models.Post, err error) {
	idString := strings.Join(ids, ",")
	sqlStr := `select post_id, title, content, author_id, community_id, like_count, create_time
				from post
				where post_id in ?
				order by FIND_IN_SET(post_id, ?)`
	err = db.WithContext(ctx).Raw(sqlStr, ids, idString).Scan(&postList).Error
